...
/**
 * @ast node
 * @declaredat C:\\Users\\vishnu\\Documents\\GitHub\\SQL-injection-detection\\spec\\php.ast:24
 * @production SqlExpr : {@link Expr} ::= <span class="component">Left:{@link Expr}</span> <span class="component">Right:{@link Expr}</span>;

 */
public class SqlExpr extends Expr implements Cloneable {
  /**
   * @apilevel internal
   */
  public SqlExpr clone() throws CloneNotSupportedException {
    SqlExpr node = (SqlExpr) super.clone();
    node.FV_visited = -1;
    node.RV_visited = -1;
   ...
   * @apilevel low-level
   */
  public SqlExpr fullCopy() {
    SqlExpr tree = (SqlExpr) copy();
    if (children != null) {
  ...
  /**
   */
  public SqlExpr(Expr p0, Expr p1) {
    setChild(p0, 0);
    setChild(p1, 1);
  }
  /**
  ...
  public String unparse() {
    ...
    try { return "mysqli_query(" + getLeft().unparse()+ ", " + getRight().unparse()+")"; }
    finally {
      unparse_visited = -1;
    }
  }
