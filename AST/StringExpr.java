...
/**
 * @ast node
 * @declaredat C:\\Users\\vishnu\\Documents\\GitHub\\SQL-injection-detection\\spec\\php.ast:22
 * @production StringExpr : {@link Expr} ::= <span class="component">Left:{@link Expr}</span> <span class="component">Op:{@link Op_s}</span> <span class="component">Right:{@link Expr}</span>;

 */
public class StringExpr extends Expr implements Cloneable {
  /**
   * @apilevel internal
   */
  public StringExpr clone() throws CloneNotSupportedException {
    StringExpr node = (StringExpr) super.clone();
    node.FV_visited = -1;
    node.RV_visited = -1;
  ...
  public StringExpr copy() {
    try {
      StringExpr node = (StringExpr) clone();
      node.parent = null;
      if(children != null) {
  ...
  public StringExpr fullCopy() {
    StringExpr tree = (StringExpr) copy();
    if (children != null) {
  ...
  /**
   */
  public StringExpr(Expr p0, Op_s p1, Expr p2) {
    setChild(p0, 0);
    setChild(p1, 1);
    setChild(p2, 2);
  }
   ...
   * @apilevel low-level
   */
  public Op_s getOpNoTransform() {
    return (Op_s) getChildNoTransform(1);
  }
  /**
